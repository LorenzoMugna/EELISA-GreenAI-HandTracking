...
# ── simulation thread ─────────────────────────────────────────────────────────
# Adding data to save
queues_to_save: list[list[float]] = [[] for _ in range(N_CHANNELS)] # Contains timestamps of spilkes
    ...
    while not stop_event.is_set():
        ...
        for _ in range(steps):
            for i in range(N_CHANNELS):
                ...
                if spiked:
                    queues_to_save[i].append(sim_time)
...
def save_queues(file) -> bool:
    try:
        with open(file, "w") as f:
            for queue in queues_to_save:
                f.write(",".join([str(t) for t in queue]) + "\n")
    except Exception as e:
        print(f"Error saving queues: {e}")
        return False
    print(f"Queues saved to {file}")
    return True
...
# ── entry point ───────────────────────────────────────────────────────────────
import argparse
parser = argparse.ArgumentParser(description="Collect neuron spike data from Leap Motion input")
parser.add_argument("--label", default="0", help="Label for the collected data (used in output filename)")
parser.add_argument("--output", "-o", default="spike_times.csv", help="Output CSV file to save spike times")
def main() -> None:
    ...
    file_base = args.output.rsplit(".", 1)[0]
    file_name = f"{file_base}_{args.label}.csv"

    with connection.open():
        try:
            run_gui()
        finally:
            save_queues(file_name)
            stop_event.set()
